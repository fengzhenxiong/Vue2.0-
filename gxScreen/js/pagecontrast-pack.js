...
Component.options.__file = "D:\\work\\MY-PROJECT\\DW.News.App.Screen\\dist\\js\\component\\spread2\\warn.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key !== "__esModule"})) {console.error("named exports are not supported in *.vue files.")}
...
var _warn = __webpack_require__(3);
...
/**
 * 每隔5秒钟切换一下对比报纸。每次加载10条报纸，完成后，隔10s再推入10条数据
 */
var vm = new Vue({
    el: "#fn-s-container",
    data: (_data = {
        ...
        papers: [],
        totalpaper: [],
        paperpushinterlval: 10000,
        paperpushtimer: null,
        interval: 5000,
        timer: null,
        per: 10
    }, _defineProperty(_data, "timer", null), _defineProperty(_data, "mainid", 1152), _data),
    computed: {
    ...
    methods: {
        ...
        //设置左侧图片
        setLeftPaper: function getdatas() {
            var self = this;
            var params = {
                loadDate: self.getToday(),
                paperName: '广西日报'
            };
            tools.queryData('get',params,'/datas/api/hotspot/newspaper/pageListTouban').then(function(response){
                var data = response.data.content[0];
                self.loadshow = false;
                self.error = false;
                self.leftpaper = {
                    id: data.sid,
                    time: data.urlTime.substr(0,10),
                    papername: data.paperName,
                    img: data.imgUrl.startsWith('http') ? data.imgUrl : 'http://' + data.imgUrl
                };
            });
        },
        //设置右侧图片
        setRightPaper: function getdata(mid, idstr) {
            var self = this;
            var params = {
                loadDate: self.getToday(),
                paperName: '广西日报;北京日报;南方日报;天津日报;新华日报;河南日报;江西日报;云南日报;吉林日报;黑龙江日报;内蒙古日报;经济日报;湖南日报;浙江日报;湖北日报;福建日报;陕西日报;辽宁日报;四川日报;河北日报;宁夏日报;重庆日报;海南日报;人民日报;青海日报;贵州日报;山西日报;安徽日报;中国青年报;解放军报;大众日报;西藏日报;新疆日报;光明日报;甘肃日报'
            };
            tools.queryData('get',params,'/datas/api/hotspot/newspaper/pageListTouban').then(function(response){
                var list = response.data.content;
                self.handledata(list)
            });
        },
        ...
        handledata: function handledata(data) {
            ...
            if ((typeof data === "undefined" ? "undefined" : _typeof(data)) == "object") {
                if (data && data.length) {
                    //返回不为空
                    self.curindex = 0;
                    // var paperindex = data.mainIndex;
                    // var list = data;
                    // self.leftpaper = {
                    //     id: list[paperindex].paperID,
                    //     time: paperDate,
                    //     papername: list[paperindex].paperName,
                    //     img: list[paperindex].LayoutSource
                    // };
                    self.totalpaper = data;
                    // // 报纸总数小于设定的每次渲染数
                    // if (self.totalpaper.length < self.per) {
                    //     self.per = self.totalpaper.length;
                    // } else {
                    //     self.addpaperbystep(); //逐步加载数据
                    // }
                    self.renderpaper(0);
                    self.animate(); // 开始报纸切换
                } else {
                    console.log("返回报纸数组为空！");
                }
            } else {
                // obj返回为null
                console.log("返回数据有问题请检查");
            }
        },

        // 分段渲染报纸
        renderpaper: function renderpaper(start) {
            var self = this,
                end = self.totalpaper.length;
            // (end > self.totalpaper.length) && (end == self.totalpaper.length);
            for (var i = start; i < end; i++) {
                var li = self.totalpaper[i];
                var id = li.sid,
                    time = li.urlTime.substr(0,10),
                    papername = li.paperName,
                    img = li.imgUrl.startsWith('http') ? li.imgUrl : 'http://' + li.imgUrl;
            ...
            self.papers = self.doubleArr(self.papers); //化为二维数组 
        },
        prev: function prev() {
            this.stop();
            if (this.curindex == 0) {
        ...
        // addpaperbystep() { //分步加载
        //     var self = this;
        //     self.paperpushtimer = setInterval(function() {
        //         if (self.papers.length < self.totalpaper.length) {
        //             self.renderpaper(self.papers.length);
        //         } else {
        //             clearInterval(self.paperpushtimer);
        //         }
        //     }, self.paperpushinterlval);
        // },
        doubleArr: function doubleArr(arr) {
            //二维数组
            var self = this;
            var totalarr = [];
            ...
            for (var i = 1; i <= num; i++) {
                ...
                for (var j = (i - 1) * 4; j < i * 4; j++) {
                    if (self.papers[j] != null) {
                        newarr.push(self.papers[j]);
                    }
                }
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(15)("eef01f9c", content, false);
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
